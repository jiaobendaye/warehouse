...
// Check — POST /api/v1/replenishment/check  body: { "skus": [...], "policy": "..." }
func (h *ReplenishmentHandler) Check(w http.ResponseWriter, r *http.Request) {
	...
	if err := decodeJSON(r, &req); err != nil {
		...
		// helpful error.
		WriteError(w, http.StatusBadRequest, "BAD_REQUEST", "request body must be {skus:[], policy:\"\"}: "+err.Error())
		return
	}
	res, err := h.svc.Check(r.Context(), req.SKUs, req.Policy)
	if err != nil {
		status, code := TranslateError(err)
		WriteError(w, status, code, err.Error())
		return
